...
export function LightningReceivePanel(): React.JSX.Element
{
    ...
    const amountSats = /^\d+$/.test(amountText.trim()) && amountText.trim() !== "0"
        ...
        : null;
    const overCap = amountSats !== null && amountSats > LN_EXPERIMENTAL_MAX_SATS;

    const canCreate = isOnline && Boolean(account) && amountSats !== null && !createMutation.isPending && !receive && !lowSol && !overCap;
