...
# 격자 회색의 RGB 분포: 채도(max-min)는 작고, 명도는 100~150 정도 (실측 기준).
# 매우 보수적으로 잡아 코인 ring의 anti-aliased 경계(명도 170+)는 절대 침범하지 않음.
SAT_THRESHOLD = 12          # 거의 pure gray만 (R≈G≈B 엄격)
BRIGHTNESS_LOW = 60         # 이보다 어두우면 검정 디테일 보존
BRIGHTNESS_HIGH = 165       # 이보다 밝으면 흰색 ring의 anti-alias 경계로 간주, 보존
